...
function getInfoData(user, casename){
    ...
    let fwm = localStorage.getItem("fwm");
...
function FORMOR(user, casename){
    ...
    function init(DATA){
        ...
        //       let weeksarray = range(0, 52, $step=1)
        let weeks = [
            {"id":0, "name":"Not scheduled"},
        ];
        for(i = 1; i < 53; i++) {
            weeks.push({"id": i, "name": i.toString()})
        };
//            weeks.push({"id":element, "name":element.toString()})
//        };

//            {"id":0, "name":"Not scheduled"},
//            {"id":1, "name":"1"},

//        weeks[0] = "Not scheduled"
        ...
        var weekSource = {
            localdata: JSON.stringify(weeks),
            datatype: "json",
            datafields:
                [
                    { name: 'id', type: 'number' },
        ...
        var FWMweeks = new $.jqx.dataAdapter(weekSource, {autoBind: true});
        ...
        var src ={
            ...
            datafields: [
                ...
                { name: 'FWM', value: 'FWMId', values: { source: FWMweeks.records, value: 'id', name: 'name' } },
                { name: 'FWMId', type: 'number'},
        ...
        var weekEditor = function(row, value, editor) {
            editor.jqxDropDownList({ source: FWMweeks, displayMember: 'name', valueMember: 'id' });
        }
        ...
        $("#jqxGridFORMOR").jqxGrid({
            ...
            columns: [
                ...
                { text: 'First week of maintanance (1-52)', datafield: 'FWMId', displayfield: 'FWM', cellsalign: 'right', align: 'right', columntype: 'dropdownlist',  createeditor: weekEditor},
...
function saveFORMOR() {
    ...
    $.each( data, function( id, obj ) {
        ...
        FORMORData['FWM'][obj['Tech']] = obj['FWMId'];
    });
        ...
        complete: function(e) {
            ...
            switch (serverResponce["type"]) {
                ...
                case 'SUCCESS':
                    ...
                    localStorage.setItem("fwm",  "changed");
                    mdChangedMsg();
                    $('#FORMOR').modal('toggle');
                    break;
            }
